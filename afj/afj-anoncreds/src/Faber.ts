import type { AnonCredsCredentialDefinition, RegisterCredentialDefinitionReturnStateFinished, RegisterSchemaReturnStateFinished } from '@aries-framework/anoncreds'
import type { ConnectionRecord, ConnectionStateChangedEvent, CredentialStateChangedEvent } from '@aries-framework/core'
import type {
...
import { ConnectionEventTypes, CredentialEventTypes, CredentialState, KeyType, TypedArrayEncoder, utils } from '@aries-framework/core'
import { ui } from 'inquirer'

import { BaseAgent, indyNetworkConfig } from './BaseAgent'
import { Color, Output, greenText, purpleText, redText } from './OutputClass'
import { data_size, timing_faber, timings } from './stats-time'
...
export class Faber extends BaseAgent {
  ...
  public async issueCredential(curr_cred_num: number) {
    ...
    await this.agent.credentials.offerCredential({
      ...
      protocolVersion: 'v2',
      // data_size + 20 to apprx compensate for less vc fields
      credentialFormats: {
        anoncreds: {
          attributes: [
            {
              name: 'name',
              value: 'Alice Smith' + curr_cred_num.toString(),
            },
            {
              name: 'degree',
              value: 'a'.repeat(data_size + 20),
            },
            {
              name: 'date',
              value: '01/01/2022',
    ...
    // appendFileSync('.stats/time_total.txt', time_total.toString() + '\n');
